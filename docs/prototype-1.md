...
them in Terminal. **They are not additional CSV or SQLite columns**, so record
the observed decision in your trial notes if you need it for comparison.
The saved 22-column measurements remain available for analysis.
...
4. Allow startup to settle, then perform the intended movement. Leave enough
   ...
   The recorder still stops after **30 seconds** by default; use `--duration 60`
   if a trial needs more time.
...
accuracy on new trials or actual human falls. Hardware validation remains
pending until you rebuild, upload and test the board.
